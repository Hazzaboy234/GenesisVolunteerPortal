...
export class Login extends Component {
    ...
    //Gets the cookie data does some stuff with it
    setup() {
        var cookie = this.props.getCookie();
        if (cookie != null) {
            var string = "Are you 1 2?".replace("1", cookie.firstName).replace("2", cookie.secondName)
            if (window.confirm(string) === true) (window.location.replace("/dashboard"))
        } else {
